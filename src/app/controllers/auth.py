...
def activate():
    ...
    if request.method == 'POST':
        key = (request.form.get('activation_key') or "").strip()

        # IMPORTANT: always save/read license from instance folder (beside EXE)
        save_license(current_app.instance_path, key)
        is_valid, msg = verify_license(current_app.instance_path)
