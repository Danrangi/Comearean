...
def download_result(result_id):
    ...
    width, height = letter
    # Best-effort logo (won't crash if missing)
    try:
        logo_path = os.path.join(current_app.static_folder, 'img', 'logo.png')
        if os.path.exists(logo_path):
            p.drawImage(logo_path, width - 120, height - 80, width=60, height=60, mask='auto')
    except Exception:
        pass
    p.setFont("Helvetica-Bold", 24)
    p.drawString(50, height - 50, "Exam Arena CBT Practice Result Slip")
    ...
    p.drawString(50, 50, "Generated by Exam Arena CBT Practice. This slip is for practice only (not official).")
